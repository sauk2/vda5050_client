...
TEST(CallbackRegistryTest, SingleUpdateQuery)
{
  vda5050_execution::CallbackRegistry registry;
  ...
  registry.register_provider<UpdateA>(
    [&](std::shared_ptr<UpdateA>) { call_count++; });
  registry.register_provider<UpdateB>(
    [&](std::shared_ptr<UpdateB>) { wront_type_count++; });
...
TEST(CallbackRegistryTest, MultiUpdateQuery)
{
  vda5050_execution::CallbackRegistry registry;
  ...
  registry.register_provider<UpdateA>(
    [&](std::shared_ptr<UpdateA>) { call_count_1++; });
  registry.register_provider<UpdateB>(
    [&](std::shared_ptr<UpdateB>) { call_count_2++; });

  auto update_a = std::make_shared<UpdateA>();
  registry.query(update_a);

  auto update_b = std::make_shared<UpdateB>();
  registry.query(update_b);
...
TEST(CallbackRegistryTest, EmptyCalls)
{
  ...
  auto update = std::make_shared<UpdateA>();
  EXPECT_NO_THROW(registry.query(update));
}
